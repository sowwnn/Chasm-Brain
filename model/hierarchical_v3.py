...
class MambaStage2(nn.Module):
    """
    Stage 2 V3: Mamba Fine Stage with Hybrid Global + Local.
    ...
    def __init__(
        ...
        voxels_per_cluster=30,
        embed_dim=384,
        latent_dim=128,  # kept for compatibility but not used
        depth=6,
        dropout=0.1,
...
class HierarchicalARM_V3(nn.Module):
    ...
    def forward(self, cls_token, patch_tokens, stage=2, gt_roi_means=None):
        ...
        Returns:
            ...
            If stage 2: (guidance, pred_voxels)
        """
...
if __name__ == "__main__":
    ...
    model.eval()
    with torch.no_grad():
        guidance, pred_voxels = model(cls_token, patch_tokens, stage=2)
    print(f"Stage 2: guidance={guidance.shape}, pred_voxels={pred_voxels.shape}")
